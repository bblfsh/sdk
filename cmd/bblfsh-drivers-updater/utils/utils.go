...
func newPipeLine(d discovery.Driver, githubToken string, o *UpdateOptions) *pipeLine {
	url := d.RepositoryURL()
	origin := getOrigin(url, o.Dockerfile, githubToken)
...
// StringIsEmpty checks if string is empty or contains only whitespaces
func StringIsEmpty(s string) bool { return len(strings.TrimSpace(s)) == 0 }

func getOrigin(url string, isDockerfile bool, githubToken string) string {
	token := "${GITHUB_TOKEN}"
	if !isDockerfile {
		token = githubToken
	}

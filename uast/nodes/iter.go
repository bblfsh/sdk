...
const (
	...
	ChildrenOrder
	// OrdersCount is a special constant value for the number of known iteration orders.
	// It is not valid order and must not be used as an argument to iterator functions.
	OrdersCount
)
...
func newChildrenIterator(n External) Iterator {
	...
	eachChild(n, func(v External) {
		...
		case KindArray:
			if arr, ok := v.(ExternalArray); ok {
				for i := 0; i < arr.Size(); i++ {
					nodes = append(nodes, arr.ValueAt(i))
				}
			}
		}
	})
